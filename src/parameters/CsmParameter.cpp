...
namespace lapis {
	...
	bool CsmParameter::prepareForRun() {
		...
		if (_footprintDiameter.getValueLogErrors() < 0) {
			log.logWarningOrError("Pulse diameter must be non-negative");
			return false;
		}
		if (_cellsize.getValueLogErrors() <= 0) {
			log.logWarningOrError("CSM Cellsize must be positive");
			return false;
		}
		...
		if (_footprintDiameter.getValueLogErrors() >= _cellsize.getValueLogErrors() * 2) {
			log.logWarningOrError("Pulse diameter is too large for your CSM resolution. Increase CSM cellsize or reduce pulse diameter.");
		}
